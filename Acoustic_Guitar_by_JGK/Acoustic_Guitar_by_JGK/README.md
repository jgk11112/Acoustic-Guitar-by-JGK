...
## v0.4 real-sample upgrade
The build now downloads and embeds 15 CC0 recordings of a 2017 Martin HD28 Vintage Series from the Discord SFZ GM Bank at a pinned upstream commit. The real recordings are the main tone; a very quiet physical-model layer only adds variation/fallback. Palm mute is produced by damping and darkening the real recording in real time. See `THIRD_PARTY_SAMPLES.md` for provenance.
